...
function customerSuccessBalancing(
  ...
  if (customerSuccess.length === 1) return customerSuccess[0].id

  const customerSuccessReady = customerSuccess
    .filter(
  ...
  return customerSuccessReady[0].served === customerSuccessReady[1].served ? 0 : customerSuccessReady[0].id;
}
...
test("Scenario 2", () => {
  ...
  expect(customerSuccessBalancing(css, customers, csAway, true)).toEqual(0);
});
...
test("Scenario 8", () => {
  ...
  const csAway = [2, 4];
  expect(customerSuccessBalancing(css, customers, csAway, true)).toEqual(1);
});
